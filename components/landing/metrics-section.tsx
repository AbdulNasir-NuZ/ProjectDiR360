    ...
    <section ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-350 mx-auto px-6 lg:px-12">
          ...
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-sm font-mono text-muted-foreground">
                {time
                  ? `${time.toLocaleTimeString("en-GB", { timeZone: "UTC", hour12: false })} UTC`
                  : "--:--:-- UTC"}
              </span>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {/* Large metric */}
          <div className={`lg:col-span-1 bg-foreground/2 border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
          ...
          {metrics.slice(1).map((metric, index) => (
            <div
              key={metric.label}
              className={`bg-foreground/2 border border-foreground/10 p-8 flex flex-col items-start justify-between gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
